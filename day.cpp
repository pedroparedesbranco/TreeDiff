...
void get_string(FILE* fp, char c, int size, int num_tree, int &leaf_count, unordered_map<string, int> &code, vector<int> *sizes, vector<string> *labels){
    string str = "";
    while(!(c == ')' || c == '(' || c == ',' || c == ';')){
        str.append(1, c);
        c = getc(fp);
...
int create_table(FILE* fp, int size, int num_tree, int &leaf_count, int &internal_count, unordered_map<string, int> &code, vector<int> *sizes, vector<string> *labels){
    char c;
    string str;
    while((c=getc(fp)) != ';'){
        // cout << c << endl;
        if(c == '('){
            size = size + create_table(fp, 0, num_tree, leaf_count, internal_count, code, sizes, labels);
        }
        else if(c == ')'){
            sizes[num_tree].push_back(size);
            ...
            get_string(fp, c, 0, num_tree, leaf_count, code, sizes, labels);
            size++;
        }
    }
    return -1;
...
void create_clusters(int num_nodes1, int num_leafs, unordered_map<string, int> &code, vector<int> *sizes, vector<string> *labels, vector<int> *clusters){
...
void compute_distance(int num_nodes2, int num_leafs, unordered_map<string, int> &code, vector<int> *sizes, vector<string> *labels, vector<int> *clusters, int &rf_dist){
    stack<Struct> stack;
    Struct first, second;
    int i;
    int enc, weight;
...
int main(int argc, char* argv[]){
    ...
    create_table(fp, 0, 0, leaf_count1, internal_count1, code, sizes, labels);
    ...
    create_table(fp, 0, 1, leaf_count2, internal_count2, code, sizes, labels);
    ...
    create_clusters(num_nodes1, leaf_count1, code, sizes, labels, clusters);
