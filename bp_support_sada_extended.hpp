...
*/
/*! \file bp_support_pedro2.hpp
    \brief bp_support_pedro2.hpp contains an implementation of a balanced
     parentheses support structure proposed by Kunihiko Sadakane.
    \author Simon Gog
*/
#ifndef INCLUDED_SDSL_BP_SUPPORT_PEDRO2
#define INCLUDED_SDSL_BP_SUPPORT_PEDRO2
...
//#define USE_CACHE
        ...
        void copy(const bp_support_pedro2& bp_support)
        {
            m_bp        = bp_support.m_bp;
            m_bp_rank   = bp_support.m_bp_rank;
            m_bp_rank.set_vector(m_bp);
            m_bp_rank_10   = bp_support.m_bp_rank_10;
    ...
    public:
        ...
        //! Constructor
        explicit bp_support_pedro2(const bit_vector* bp): m_bp(bp),
        ...
        //! Copy constructor
        bp_support_pedro2(const bp_support_pedro2& bp_support)
        {
            copy(bp_support);
        }

        //! Move constructor
        bp_support_pedro2(bp_support_pedro2&& bp_support)
        {
            *this = std::move(bp_support);
        }

        //! Assignment operator
        bp_support_pedro2& operator=(bp_support_pedro2&& bp_support)
        {
            if (this != &bp_support) {
                m_bp        = std::move(bp_support.m_bp);
                m_bp_rank   = std::move(bp_support.m_bp_rank);
                m_bp_rank.set_vector(m_bp);
                m_bp_rank_10   = std::move(bp_support.m_bp_rank_10);
                m_bp_rank_10.set_vector(m_bp);
         ...
         */
        void swap(bp_support_pedro2& bp_support)
        {
        ...
        //! Assignment operator
        bp_support_pedro2& operator=(const bp_support_pedro2& bp_support)
        {
            if (this != &bp_support) {
                copy(bp_support);
            }
        ...
        /*
                bp_support_pedro2& operator=(bp_support_pedro2&& bps)
                {
                    this->swap(bps);
                    return *this;
                }
        */
        ...
        /*! Returns the number of "10" up to and including index i.
         * \pre{ \f$ 0\leq i < size() \f$ }
         */
        size_type rank10(size_type i)const
        {
            return m_bp_rank_10(i+1);
        }

        /*! Returns the index of the i-th closing parenthesis.*/
        size_type select0(size_type i)const
        {
            return m_bp_select_0(i);
        }

        /* Returns the value that is in position "pos".*/
        size_type bv_value(size_type pos){
            return (this->m_bp)[0][pos]; 
        }

        // size_type sml_block_idx1(size_type v){
        //     return sml_block_idx(v);
        // }

        // size_type med_block_idx1(size_type v){
        //     return med_block_idx(v);
        // }

        // size_type is_left_child1(size_type v)
        // {
        //     return is_left_child(v);
        // }

        // size_type left_child1(size_type v)
        // {
        //     return left_child(v);
        // }

        /* Returns the position where "v" first child is located.*/
        size_type first_child(size_type v){
        ...
        /* Returns the position where "v" last child is located.*/
        size_type last_child(size_type v){
            if(this->m_bp[0][v+1] == 1){
                return this->find_open(this->find_close(v) - 1);
            }
            else{
                return -1;
            }
        ...
        /* Returns the position where "v" next sibling is located.*/
        size_type next_sibling(size_type v){
            int i = this->find_close(v) + 1;
            if(v != 0 && this->m_bp[0][i] == 1){
                return i;
            }
            else{
                return -1;
        ...
        /* Returns the position where "v" previous sibling is located.*/
        size_type prev_sibling(size_type v){
            if(v > 1 && this->m_bp[0][v-1] == 0){
        ...
        /* Verifies if node located in position "v" is a leaf.*/
        size_type isleaf(size_type v){
            return this->m_bp[0][v+1] == 0;
        }

        /* Returns the pre order index of node located at position "v".*/
        size_type nodemap(size_type v){
            return this->rank(v);
        }

        /* Returns the position where node with pre order index "i" is located.*/
        size_type nodeselect(size_type i){
        ...
        /* Returns the position where node with post order index "i" is located.*/
        size_type post_order_select(size_type i){
            return this->find_open(this->select0(i));
        }

        /* Returns the depth of node located in position "v".*/
        size_type depth(size_type v){
            return this->excess(v);
        }

        // size_type subtree(size_type v){
        //     return int((this->find_close(v) - v + 1)/2);
        // }

        // size_type is_ancestor(size_type u, size_type v){
        //     return u <= v <= this->find_close(u);
        // }

        // size_type level_ancestor(bp_support_ggg<> vec, size_type v, size_type d){
        ...
        /* Returns the position where the lca between nodes in positions "u" and "v" are located.*/
        size_type lca(size_type u, size_type v){
            int aux;
            if(u > v){
                aux = v;
                v = u;
            ...
            // std::cout << u << std::endl;
            // std::cout << v << std::endl;
            // std::cout << this->rmq(u, v) << std::endl;
            return this->enclose(this->rmq(u, v) + 1);
        }

        // size_type is_last_sib(bp_support_ggg<> vec, bit_vector m_bp, size_type v){
        //     if(v == 0){
        //         return 1;
        //     }
        //     if(isleaf(vec, m_bp, v)){
        //         return (m_bp[v+1] == 0 && m_bp[v+2] == 0);
        //     }
        //     size_type x = vec.find_close(v);
        //     return m_bp[x+1] == 0;
        // }

        /* Returns the number of nodes below "v".*/
        size_type cluster_size(size_type v){
            // return this->rank(this->find_close(v)) - this->rank(v) + 1;
            return (this->find_close(v) - v + 1) / 2;
        }

        /* Returns the number of nodes betwen "v" and "u".*/
        size_type cluster_size2(size_type v, size_type u){
            // return this->rank(u) - this->rank(v) + 1;
            return (u - v + 1) / 2;
        }

        /* Returns the number of leaves below "v".*/
        size_type num_leaves(size_type v){
            return this->rank10(this->find_close(v)) - this->rank10(v);
        }

        /* Returns the number of leaves betwen "v" and "u".*/
        size_type num_leaves2(size_type v, size_type u){
            return this->rank10(u) - this->rank10(v);
        }

        /* Retuens the position where "v" parent is located.*/
        ...
        */
        ...
        //! Serializes the bp_support_pedro2 to a stream.
        /*!
        ...
        //! Load the bp_support_pedro2 for a bit_vector v.
        /*!
