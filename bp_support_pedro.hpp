...
/*! \file bp_support_gg.hpp
    ...
    \author Simon Gog
*/
#ifndef INCLUDED_SDSL_BP_SUPPORT_PEDRO
#define INCLUDED_SDSL_BP_SUPPORT_PEDRO
...
template<class t_nnd = nearest_neighbour_dictionary<30>,
         ...
         uint32_t t_bs=840>
class bp_support_pedro
{
        static_assert(t_bs > 2, "bp_support_pedro: block size must be greater than 2.");
    public:
        ...
        typedef bp_support_pedro<nnd_type, rank_type, rank_type_10, select_support_scan<>, t_bs> bp_support_type;
    private:
        const bit_vector* m_bp;         // balanced parentheses sequence
        rank_type         m_rank_bp;    // rank support for m_bp => see excess() and rank()
        rank_type_10         m_rank_bp_10;    // rank support for m_bp => see excess() and rank()
        ...
        void copy(const bp_support_pedro& bp_support)
        {
            m_bp = bp_support.m_bp;
            m_rank_bp = bp_support.m_rank_bp;
            m_rank_bp.set_vector(m_bp);
            m_rank_bp_10 = bp_support.m_rank_bp_10;
            m_rank_bp_10.set_vector(m_bp);
    ...
    public:
        ...
        const rank_type_10& bp_rank_10;
        const select_type& bp_select;

        bp_support_pedro(): m_bp(nullptr), m_pioneer_bp_support(nullptr),
            m_size(0), m_blocks(0),bp_rank(m_rank_bp),bp_rank_10(m_rank_bp_10),
            bp_select(m_select_bp) {}

        //! Constructor
        explicit bp_support_pedro(const bit_vector* bp):m_bp(bp),
            m_pioneer_bp_support(nullptr),
            m_size(bp==nullptr?0:bp->size()),
            m_blocks((m_size+t_bs-1)/t_bs),
            bp_rank(m_rank_bp),bp_select(m_select_bp),bp_rank_10(m_rank_bp_10)
        {
            if (bp == nullptr) { // -> m_bp == nullptr
                return;
            }
            util::init_support(m_rank_bp, bp);
            util::init_support(m_rank_bp_10, bp);
        ...
        //! Copy constructor
        bp_support_pedro(const bp_support_pedro& bp_support) : bp_support_pedro()
        {
            copy(bp_support);
        }

        //! Move constructor
        bp_support_pedro(bp_support_pedro&& bp_support) : bp_support_pedro()
        {
        ...
        //! Swap operator
        void swap(bp_support_pedro& bp_support)
        {
            m_rank_bp.swap(bp_support.m_rank_bp);
            m_rank_bp_10.swap(bp_support.m_rank_bp_10);
        ...
        //! Assignment operator
        bp_support_pedro& operator=(const bp_support_pedro& bp_support)
        {
            if (this != &bp_support) {
                copy(bp_support);
            }
        ...
        bp_support_pedro& operator=(bp_support_pedro&& bp_support)
        {
            if (this != &bp_support) {
                ...
                m_rank_bp.set_vector(m_bp);
                m_rank_bp_10 = std::move(bp_support.m_rank_bp_10);
                m_rank_bp_10.set_vector(m_bp);
        ...
        size_type next_sibling(size_type v){
            ...
            if(v != 0 && this->m_bp[0][i] == 1){ //TODO: find_close
                return i;
            }
            else{
                return -1;
